...
export default function Buttons() {
    ...
    const handleSave = async () => {
        ...
        toast.loading("Курьер қосылып жатыр");
        setIsLoading(true);
        apiInstance
            .post("/auth/registerDeliveryMan", payload)
            .then(() => {
                toast.dismiss();
                toast.success("Курьер сәтті қосылды");
                data.reset();
                wait(100).then(() => window.location.reload());
            })
            .catch(() => {
                toast.dismiss();
                toast.error("Жаңа курьерді қосу кезінде қате шықты");
            })
