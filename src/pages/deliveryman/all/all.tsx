...
export default function AllDeliveryMan() {
    ...
    useEffect(() => {
        fetchCategories()
            ...
            .catch((err) => {
                console.error(err);
                toast.error("Қателер пайда болды...");
            })
    ...
    return (
        <main className="flex">
            <section className="w-full">
                <TopBar text="Барлық курьерлер"></TopBar>
                <div className="h-[80px] px-10 flex items-center  border">
                    ...
                    <Link to={"/drivers/add"} className="ml-auto">
                        <Button className="ml-auto opacity-90 bg-zinc-800 font-medium text-sm h-[40px]">
                            <PlusSquare className="stroke-[2] mr-2  w-5" /> Жаңа курьерді қосу
                        </Button>
                    </Link>
                </div>
...
async function fetchCategories() {
    const { data } = await apiInstance.get("/auth/DeliveryMan");
