...
import { useAuth } from "@/constants/AuthContext"; // Moved import to top
...
const TitleSection = ({ open }: { open: boolean }) => {
    ...
    const adminName = user?.name || user?.email || "Admin"; // Display name, fallback to email, then "Admin"
                ...
                <div className="flex items-center gap-2">
                    ...
                    {open && (
                        <motion.div
                            ...
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ delay: 0.125 }}>
                            <span className="block text-xs font-semibold truncate max-w-[120px]" title={adminName}>
                                {adminName}
                            </span>
